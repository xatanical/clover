...
bot.command("pullupdate", async (ctx) => {
  try {
    await ctx.reply("⏳ Mengecek Update Terbaru...");

    const apiURL = `https://api.github.com/repos/${GITHUB_REPO}/contents?ref=${GITHUB_BRANCH}&t=${Date.now()}`;
    ...
    if (!res.ok) return ctx.reply("❌ Tidak bisa mengakses owner.");

    const files = await res.json();
    const candidates = files.filter(f =>
      f.name.toLowerCase().includes("clover") &&
      f.name.endsWith(".js")
    );

    if (candidates.length === 0) {
 return ctx.reply("❌ Tidak ada file di owner.");
    }
    const target = candidates.sort((a, b) => b.size - a.size)[0];
    const fileURL = `${target.download_url}?nocache=${Date.now()}`;
    const fileRes = await fetch(fileURL);

    if (!fileRes.ok) {
      return ctx.reply("❌ Gagal mendownload file terbaru.");
    }
    const textContent = await fileRes.text();
    fs.writeFileSync("./clover.js", textContent);
    await ctx.reply(
      `✅ *UPDATE BERHASIL*\n` +
      `📄 File: *${target.name}*\n` +
